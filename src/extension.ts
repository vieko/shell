...
export class Ext extends Ecs.System<ExtEvent> {
    ...
    move_workspace(direction: Meta.DisplayDirection) {
        ...
        const display_move = () => {
            const monitor_id = Tiling.locate_monitor(win.meta.get_monitor(), direction);
            ...
            if (monitor_id !== null) {
                if (this.auto_tiler) {
                    ...
                    move_window(monitor_id);
                }
            }
        }

        /** Move a window normally, when not in tiling mode */
        const move_window = (monitor: number) => {
